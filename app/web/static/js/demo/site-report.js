...
function getDimensionMeta(lang) {
  return [
    ...
    },
    {
      ...
      formula: tx(
        lang,
        '综合评估 ChatGPT、Google AI Mode、AI Overviews、Perplexity 与 Gemini 的引用准备度',
        'Combined citation readiness across ChatGPT, Google AI Mode, AI Overviews, Perplexity, and Gemini'
      ),
      detail: result => {
        const scores = Object.entries(result?.platform?.platform_scores || {})
          .filter(([key]) => key.toLowerCase() !== 'grok');
...
function insightText(item, fallback = '') {
  if (typeof item === 'string' || typeof item === 'number') return String(item);
  return item?.title || item?.issue || item?.action || item?.description || item?.rationale || fallback;
}

function renderInsightCards(items, type, lang = 'zh') {
  const list = Array.isArray(items) ? items.map(item => insightText(item)).filter(Boolean).slice(0, 6) : [];
  if (!list.length) {
    return `<div class="insight-empty">${escapeHtml(type === 'risk' ? tx(lang, '暂无关键问题。', 'No key issues.') : tx(lang, '暂无快速收益建议。', 'No quick wins available.'))}</div>`;
  }
  const badge = type === 'risk' ? tx(lang, '优先处理', 'Priority') : tx(lang, '快速落地', 'Quick win');
  return `<div class="summary-insight-list ${type}">${list.map((item, index) => `
    <div class="summary-insight-item">
      <span class="summary-insight-index">${String(index + 1).padStart(2, '0')}</span>
      <p>${escapeHtml(item)}</p>
...
function localizePageType(value, lang = 'zh') {
  const key = String(value || 'page').toLowerCase().replaceAll('-', '_').replaceAll(' ', '_');
  if (lang !== 'zh') return String(value || 'Page');
  return ({
    homepage: '首页', home: '首页', about: '关于我们', service: '服务页', services: '服务页',
    product: '产品页', article: '文章页', blog: '文章页', case_study: '案例页', contact: '联系页', page: '普通页面'
  })[key] || String(value || '普通页面');
}

function platformMark(key) {
  return ({
    chatgpt: 'GPT', google_ai_mode: 'MODE', google_ai_overviews: 'AIO', perplexity: 'P', gemini: 'G'
  })[key] || String(key || '').slice(0, 3).toUpperCase();
}
...
export function renderSiteAuditReport({ task, host, lang, setCachedReportHtml }) {
  ...
  const labels = {
    ...
    platformOverviewSubtitle: tx(lang, '展示 5 个目标平台的得分、主要缺口、偏好信源与首要建议', 'Shows scores, primary gaps, preferred sources, and top recommendations across 5 platforms'),
    pageDiagnosticsTitle: tx(lang, '页面诊断', 'Page Diagnostics'),
    pageDiagnosticsSubtitle: tx(lang, '逐页展示 AI 可引用性、内容质量、技术基础、结构化数据及修复建议', 'Page-level AI citability, content, technical, structured-data, and remediation details'),
  ...
  const displayPlatformEntries = Object.entries(platformScores)
    .filter(([key]) => key.toLowerCase() !== 'grok');
  const platformHtml = displayPlatformEntries.map(([key, item]) => {
    const score = Math.max(0, Math.min(100, Number(item.platform_score ?? 0)));
    const status = scoreToStatus(score);
    const sources = Array.isArray(item.preferred_sources) ? item.preferred_sources.filter(Boolean).slice(0, 4) : [];
    return `
      <article class="platform-readiness-card platform-${escapeHtml(key)}">
        <div class="platform-readiness-head">
          <span class="platform-mark">${escapeHtml(platformMark(key))}</span>
          <div><h5>${escapeHtml(PLATFORM_LABELS[key] || key)}</h5><small>${escapeHtml(formatStatus(status, lang))}</small></div>
          <div class="platform-score ${escapeHtml(statusTone(status))}"><strong>${escapeHtml(String(score))}</strong><span>/100</span></div>
        </div>
        <div class="platform-meter"><span style="width:${score}%"></span></div>
        <dl class="platform-detail-list">
          <div><dt>${escapeHtml(tx(lang, '优化焦点', 'Focus'))}</dt><dd>${escapeHtml(item.optimization_focus || labels.unavailable)}</dd></div>
          <div><dt>${escapeHtml(tx(lang, '主要缺口', 'Primary gap'))}</dt><dd>${escapeHtml(item.primary_gap || labels.missingGap)}</dd></div>
        </dl>
        <div class="platform-sources"><span>${escapeHtml(tx(lang, '偏好信源', 'Preferred sources'))}</span><div>${sources.length ? sources.map(source => `<b>${escapeHtml(source)}</b>`).join('') : `<b>${escapeHtml(labels.preferredSourcesFallback)}</b>`}</div></div>
        <div class="platform-recommendation"><span>${escapeHtml(tx(lang, '首要建议', 'Top recommendation'))}</span><p>${escapeHtml((item.key_recommendations || [])[0] || labels.recommendationFallback)}</p></div>
      </article>
    `;
  }).join('');
  ...
  const pageDiagnosticsHtml = pageDiagnostics.length
    ? `<div class="page-diagnostics-list">${pageDiagnostics.slice(0, 12).map((item, index) => {
        const overallScore = Math.max(0, Math.min(100, Number(item.overall_score ?? 0)));
        const pageStatus = item.status || scoreToStatus(overallScore);
        const metrics = [
          [tx(lang, 'AI 可引用性', 'AI Citability'), item.citability_score],
          [tx(lang, '内容质量', 'Content'), item.content_score],
          [tx(lang, '技术基础', 'Technical'), item.technical_score],
          [tx(lang, '结构化数据', 'Structured Data'), item.schema_score]
        ];
        return `
        <div class="page-diagnostic-card">
          <div class="page-diagnostic-head">
            <span class="page-diagnostic-index">${String(index + 1).padStart(2, '0')}</span>
            <div class="page-diagnostic-identity">
              <div class="page-diagnostic-title">
                <strong>${escapeHtml(localizePageType(item.page_type, lang))}</strong>
                <span class="page-diagnostic-badge">${escapeHtml(item.source === 'extended' ? tx(lang, '扩展采样', 'Extended') : tx(lang, '核心页面', 'Core'))}</span>
                <span class="page-diagnostic-badge issue">${escapeHtml(tx(lang, '问题', 'Issues'))} ${escapeHtml(String(item.issue_count ?? ((item.issues || []).length || 0)))}</span>
              </div>
              <div class="page-diagnostic-url">${escapeHtml(item.url || '-')}</div>
            </div>
            <div class="page-overall-score ${escapeHtml(statusTone(pageStatus))}"><span>${escapeHtml(tx(lang, '页面总分', 'Overall'))}</span><strong>${escapeHtml(String(overallScore))}</strong><small>/100 · ${escapeHtml(formatStatus(pageStatus, lang))}</small></div>
          </div>
          <div class="page-diagnostic-metrics">
            ${metrics.map(([label, value]) => {
              const score = Math.max(0, Math.min(100, Number(value ?? 0)));
              return `<div class="page-diagnostic-metric"><div><span class="label">${escapeHtml(label)}</span><strong class="value">${escapeHtml(String(score))}</strong></div><div class="page-metric-meter"><span style="width:${score}%"></span></div></div>`;
            }).join('')}
          </div>
          <div class="page-diagnostic-panels">
            <div class="page-diagnostic-panel issues">
              <h6><i></i>${escapeHtml(tx(lang, '发现的问题', 'Findings'))}</h6>
              ${formatDetailMap(item.issue_details, labels.pageDetailEmpty)}
            </div>
            <div class="page-diagnostic-panel recommendations">
              <h6><i></i>${escapeHtml(tx(lang, '修复建议', 'Recommendations'))}</h6>
  ...
  const strongestPlatform = [...displayPlatformEntries].sort((a, b) => (b[1]?.platform_score || 0) - (a[1]?.platform_score || 0))[0];
  const weakestPlatform = [...displayPlatformEntries].sort((a, b) => (a[1]?.platform_score || 0) - (b[1]?.platform_score || 0))[0];
  const noteText = [
  ...
  const html = `
    ...
    <div class="report-grid-2 report-insights-grid">
      <section class="report-section summary-insight-section risk"><div class="report-section-hdr"><div><span class="insight-kicker">RISK PRIORITY</span><h4>${escapeHtml(labels.keyIssuesTitle)}</h4></div><span>${escapeHtml(labels.keyIssuesSubtitle)}</span></div><div class="report-section-body">${renderInsightCards(topIssues, 'risk', lang)}</div></section>
      <section class="report-section summary-insight-section win"><div class="report-section-hdr"><div><span class="insight-kicker">QUICK WINS</span><h4>${escapeHtml(labels.quickWinsTitle)}</h4></div><span>${escapeHtml(labels.quickWinsSubtitle)}</span></div><div class="report-section-body">${renderInsightCards(quickWins, 'win', lang)}</div></section>
    </div>

    <section class="report-section platform-overview-section">
    ...
    <section class="report-section page-diagnostics-section">
