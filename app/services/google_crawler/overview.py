...
def build_crawler_overview(
    ...
    indexable = indexability.get("indexable") is True
    ...
    if crawl_allowed is False or not indexable or not http_ok or not render_http_ok:
        seo_status = "failed"
        seo_label = "不符合基础 SEO 收录标准"
        seo_detail = "页面存在抓取、HTTP、索引或渲染阻断，可能无法正常进入 Google 收录流程。"
    ...
    index_label = "允许索引" if indexable else "不可索引"
    summary = (
    ...
    return {
        ...
        "indexing": {
            "status": "passed" if indexable else "failed",
            "label": index_label,
            "detail": (
